...
def get_deltam(current_result, STL_deltam):
    ...
    #              1:{'ndcg_1': 1, 'ndcg_5': 1, 'ndcg_10': 1, 'ndcg_30': 1}}

    delta_m_values = []
    for key in STL_deltam:
        for metric in STL_deltam[key]:
            delta_m_values.append((-1) * (current_result[key][metric] - STL_deltam[key][metric]) / STL_deltam[key][metric])

    # Convert to numpy array and take the mean
    delta_m = np.mean(delta_m_values) * 100.0
    return delta_m
