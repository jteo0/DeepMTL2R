...
def run_gradient_experiment(choice, ckpt):
    ...
    rd = os.path.join("result", "final_experiment", "gradient_dynamics", exp_tag)
...
def main():
    ...
    if c == "1":
        if "matryoshka.pkl" in found_checkpoints:
            ckpt = found_checkpoints["matryoshka.pkl"]
            print(f"Selected: {ckpt}")
        else:
            print("[ERROR] Matryoshka checkpoint not found.")
            sys.exit(1)
    elif c == "2":
        if "feature_gating.pkl" in found_checkpoints:
            ckpt = found_checkpoints["feature_gating.pkl"]
            print(f"Selected: {ckpt}")
        else:
        ...
        if "deepmtl2r.pkl" in found_checkpoints:
            ckpt = found_checkpoints["deepmtl2r.pkl"]
            print(f"Selected: {ckpt}")
        else:
            print("[ERROR] Baseline checkpoint (deepmtl2r.pkl) not found.")
            sys.exit(1)

    run_gradient_experiment(c, ckpt)
