...
def run_optimizer_experiment(experiment_choice, checkpoint_path):
    ...
    results_dir = os.path.join(
        "result", "final_experiment", "optimizer_comparison", experiment_tag
    )
...
def main():
    ...
    available_checkpoints = {
        "deepmtl2r": {
            "subdir": "deepmtl2r",
            "filename": "deepmtl2r.pkl",
            "description": "Baseline DeepMTL2R",
        },
        "matryoshka": {
            "subdir": "matryoshka",
            "filename": "matryoshka_weight0.pkl",
            "description": "Matryoshka MRL",
        },
        "feature_gating": {
            "subdir": "feature_gating",
            "filename": "feature_gating_weight0.pkl",
    ...
    for ckpt_key, ckpt_info in available_checkpoints.items():
        ckpt_path = os.path.join(CHECKPOINT_DIR, ckpt_info["subdir"], ckpt_info["filename"])
        if os.path.exists(ckpt_path):
            found_checkpoints[ckpt_key] = ckpt_path
            print(f"  ✓ {ckpt_key:25} ({ckpt_info['description']})")
        else:
            print(f"  ✗ {ckpt_key:25} (not found)")
    ...
    # If user selected baseline, use deepmtl2r checkpoint. Otherwise, use the selected one.
    if experiment_tag == "baseline":
        if "deepmtl2r" in found_checkpoints:
            checkpoint_path = found_checkpoints["deepmtl2r"]
            print(f"Selected: {checkpoint_path}")
        else:
            print("[ERROR] Baseline checkpoint (deepmtl2r) not found.")
            sys.exit(1)
    else:
        # For extension experiments, allow user to choose
        print(f"Default for {experiment_tag}: {available_checkpoints[experiment_tag]['filename']}")

        default_ckpt = os.path.join(
            CHECKPOINT_DIR,
            available_checkpoints[experiment_tag]["subdir"],
            available_checkpoints[experiment_tag]["filename"],
        )
        if experiment_tag in found_checkpoints or os.path.exists(default_ckpt):
            checkpoint_path = found_checkpoints.get(experiment_tag, default_ckpt)
            print(f"Selected: {checkpoint_path}")
        else:
            print(
                f"\n[WARNING] Preferred checkpoint ({available_checkpoints[experiment_tag]['filename']}) not found in {CHECKPOINT_DIR}"
            )
            # Use first available checkpoint
            checkpoint_path = list(found_checkpoints.values())[0]
            print(f"Using first available: {checkpoint_path}")

    if not os.path.exists(checkpoint_path):
        print(f"\n[ERROR] Checkpoint not found: {checkpoint_path}")
        sys.exit(1)

    run_optimizer_experiment(choice, checkpoint_path)
